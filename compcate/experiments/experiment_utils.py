...
def rmse(true, pred, normalized=False):
    if not normalized:
        return np.sqrt(np.mean((pred - true) ** 2))
    else:
        return np.sqrt(np.mean((pred - true) ** 2)) / (np.std(true) + EPS)


# Taken from: https://github.com/AliciaCurth/CATENets/blob/main/catenets/datasets/network.py
