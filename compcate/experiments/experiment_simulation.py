...
from compcate.dgp.data_utils import \
    ...
    weights_from_underlying_model, rmst_from_hazard, get_survivors_from_data
from compcate.dgp.simulation_building_blocks import constant_event_model, \
    propensity_constant, propensity_linear, \
    sim_discrete_time_survival_treatment_setup, binary_covariate_model, binary_hazard_model
...
def run_experiment(ts=[1, 2, 5, 8, 10, 15, 20, 30],
                   n_exp=10,
                   n_train=200,
                   n_test=10000,
                   d=2,
                   t_max=30,
                   file_name='test', save_file=False,
    ...
    for seed in range(n_exp):
        for vary_val in vary_values:
            ...
            X, a, t, delta = sim_discrete_time_survival_treatment_setup(n=n_train + n_test, d=d,
                                                                        t_max=t_max,
            ...
            for name in models_to_fit:
                ...
                if name == 'nv':
                    ...
                    X_cf_elim, a_cf_elim, t_cf_elim, delta_cf_elim = \
                        sim_discrete_time_survival_treatment_setup(n=n_train, d=d, t_max=t_max,
                                                                   event_model=event_model,
                    ...
                    elim_cf = TSepHazardModel(fit_comp=False,
                                              model_t_params=model_params_t,
                                              model_c_params=model_params_c)
                    elim_cf.fit(X_cf_elim[:n_train, :], t_cf_elim[:n_train],
                                delta_cf_elim[:n_train], a_cf_elim[:n_train])
                    ...
                    if 'haz' in metrics:
                        ...
                        X_cf_elim_test, a_cf_elim_test, t_cf_elim_test, delta_cf_elim_test = \
                            sim_discrete_time_survival_treatment_setup(n=n_test, d=d, t_max=t_max,
                                                                       event_model=event_model,
                                                                       competing_model=constant_event_model,
                                                                       return_short=True,
                                                                       return_long=False,
                    ...
                    X_cf_sep, a_cf_sep, t_cf_sep, delta_cf_sep = \
                        sim_discrete_time_survival_treatment_setup(n=n_train, d=d, t_max=t_max,
                                                                   event_model=event_model,
                                                                   competing_model=comp_model,
                                                                   return_short=True,
                    ...
                    if 'haz' in metrics:
                        # also generate counterfactual test data to have a population to evaluate
                        # hazard against later
                        X_cf_sep_test, a_cf_sep_test, t_cf_sep_test, delta_cf_sep_test = \
                            sim_discrete_time_survival_treatment_setup(n=n_test, d=d, t_max=t_max,
                                                                       event_model=event_model,
                                                                       competing_model=comp_model,
                                                                       return_short=True,
                    ...
                    X_cf_cs, a_cf_cs, t_cf_cs, delta_cf_cs = \
                        sim_discrete_time_survival_treatment_setup(n=n_train,
                                                                   d=d,
                                                                   t_max=t_max,
                                                                   event_model=event_model,
                                                                   competing_model=comp_model,
                    ...
                    if 'haz' in metrics:
                        # also generate counterfactual test data to have a population to evaluate
                        # hazard against later
                        X_cf_cs_test, a_cf_cs_test, t_cf_cs_test, delta_cf_cs_test = \
                            sim_discrete_time_survival_treatment_setup(n=n_test,
                                                                       d=d,
                                                                       t_max=t_max,
                                                                       event_model=event_model,
                                                                       competing_model=comp_model,
                    ...
                    if with_prop:
                        ...
                        _, prop = treatment_assn_model(X, return_p=True, **treatment_model_params)
                    ...
                    _, true_prop = treatment_assn_model(X, return_p=True, **treatment_model_params)
            ...
            print(
                'Experiment {}, vary_value {}: Completed all model training.'.format(seed,
                                                                                     vary_val))

            # evaluate all fitted models  -------------------------------------------------------
            for t_horizon in ts:  # across different time horizons
                res_list = []
                for setting in effect_types:  # across different effect_types (elim, sep, cs)
                    for metric in metrics:  # evaluate all metrics
                        if setting == 'elim':  # risk-eliminated effects
                            if metric == 'rmse':
                                # compute ground truth
                                true_0, true_1 = \
                                    counterfactual_survival_prob_from_discrete_hazard_model(X_test,
                                                                                            t_horizon,
                                                                                            surv_type='risk-eliminated',
                                                                                            event_model=event_model,
                                                                                            comp_model=comp_model,
                                ...
                                # compute ground truth
                                true_0, true_1 = rmst_from_hazard(X_test, t_horizon,
                                                                  surv_type='risk-eliminated',
                                                                  event_model=event_model,
                                                                  comp_model=comp_model,
                                                                  event_model_params=event_params,
                                ...
                                X_cf_0_test, X_cf_1_test = get_survivors_from_data(X_cf_elim_test,
                                                                                   t_cf_elim_test,
                                                                                   delta_cf_elim_test,
                                                                                   a_cf_elim_test,
                                                                                   t_horizon=t_horizon)
                                ...
                                true_0, true_1 = counterfactual_survival_prob_from_discrete_hazard_model(
                                    X_test,
                                    t_horizon,
                                    surv_type='risk-eliminated',
                                    event_model=event_model,
                            ...
                            if metric == 'rmse':
                                ...
                                X_cf_0_test, X_cf_1_test = get_survivors_from_data(X_cf_sep_test,
                                                                                   t_cf_sep_test,
                                                                                   delta_cf_sep_test,
                                                                                   a_cf_sep_test,
                                                                                   t_horizon=t_horizon)
                                # compute ground truth
                                true_0, _ = counterfactual_survival_prob_from_discrete_hazard_model(
                                    X_cf_0_test,
                                    t_horizon,
                                    surv_type='separable',
                                    event_model=event_model,
                                ...
                                _, true_1 = counterfactual_survival_prob_from_discrete_hazard_model(
                                    X_cf_1_test,
                                    t_horizon,
                                    surv_type='separable',
                                    event_model=event_model,
                            ...
                            if metric == 'rmse':
                                ...
                                X_cf_0_test, X_cf_1_test = get_survivors_from_data(X_cf_cs_test,
                                                                                   t_cf_cs_test,
                                                                                   delta_cf_cs_test,
                                                                                   a_cf_cs_test,
                                                                                   t_horizon=t_horizon)
                        ...
                        for name in model_names:
                            ...
                            if metric == 'rmst':
                                pred_0, pred_1 = model_dict[key].predict_rmst(
                                    X_test, t_horizon, surv_type='risk-eliminated' if
                                    setting == 'elim' else ('separable' if setting == 'sep' else
                                                            'cause-specific'))
                            elif metric == 'rmse':
                                if setting == 'elim':
                                    pred_0, pred_1 = model_dict[
                                        key].predict_risk_eliminated_survival(
                                        X_test, t_horizon, risk=risk)
                                elif setting == 'sep':
                                    pred_0, pred_1 = model_dict[
                                        key].predict_separable_survival(
                                        X_test, t_horizon, risk=risk)
                                elif setting == 'cs':
                                    pred_0, pred_1 = model_dict[
                                        key].predict_cause_specific_survivals(X_test, t_horizon,
                                                                              risk=risk)
                            elif metric == 'haz':
                                # correct test population
                                pred_0, _ = model_dict[
                                    key].predict_hazard(X_cf_0_test, t_horizon, return_comp=False)
                                _, pred_1 = model_dict[
                                    key].predict_hazard(X_cf_1_test, t_horizon, return_comp=False)
                            elif metric == 'marghaz':
                                pred_0, pred_1 = model_dict[
                                    key].predict_hazard(X_test, t_horizon, return_comp=False)
                            elif metric == 'ess':
                                ...
                                if name == 'cf':
                                    ess_0, ess_1, n_0, n_1 = model_dict[
                                        key].compute_effective_sample_size(X,
                                                                           t_cf_sep if setting ==
                                                                                       'sep' else
                                                                           (t_cf_elim if setting
                                                                                         == 'elim' else t_cf_cs),
                                                                           delta_cf_sep if setting == 'sep' else (
                                                                               delta_cf_elim if setting
                                                                                                == 'elim' else delta_cf_cs),
                                                                           a_cf_sep if setting ==
                                                                                       'sep' else (
                                                                               a_cf_elim if setting
                                                                                            == 'elim' else a_cf_cs),
                                                                           t_horizon)
...
def run_experiment_by_setting(setting=2, file_name='', n_train=5000, n_exp=10,
                              return_res: bool = False, effect_types=None):
    if effect_types is None:
        effect_types = ['cs', 'sep', 'elim']

    if setting == '1':
        # treatment has no effect, there is only confounding
        res = run_experiment(n_train=n_train,
                             event_params={'p_group_0': 0.01, 'p_group_1': 0.1,
                                           'p_a_group_0': 0, 'p_a_group_1': 0,
                                           'support_cov': 0},
                             comp_params={'p_group_0': 0.01, 'p_group_1': 0.01,
                                          'p_a_group_0': 0, 'p_a_group_1': 0,
                                          'support_cov': 0},
                             vary_model='treat',
                             vary_param='xi',
        ...
        res = run_experiment(n_train=n_train,
                             event_params={'p_group_0': 0.01, 'p_group_1': 0.1,
                                           'p_a_group_0': 0, 'p_a_group_1': 0, 'support_cov': 0},
                             comp_params={'p_group_0': 0.01, 'p_group_1': 0.01,
                                          'p_a_group_0': 0, 'support_cov': 0},
        ...
        res = run_experiment(n_train=n_train,
                             event_params={'p_group_0': 0.01, 'p_group_1': 0.1,
                                           'p_a_group_0': 0, 'p_a_group_1': -0.09,
                                           'support_cov': 0},
                             comp_params={'p_group_0': 0.01,
                                          'p_a_group_0': 0, 'p_a_group_1': 0,
                                          'support_cov': 0},
                             vary_param='p_group_1',
                             t_max=30,
        ...
        res = run_experiment(n_train=n_train,
                             event_params={'p_group_0': 0.01, 'p_group_1': 0.1,
                                           'p_a_group_0': 0, 'p_a_group_1': 0, 'support_cov': 0},
                             comp_params={'p_group_0': 0.01, 'p_group_1': 0.01,
                                          'p_a_group_0': 0, 'p_a_group_1': 0.1, 'support_cov': 0},
                             vary_model='treat',
                             vary_param='xi',
                             t_max=30,
                             vary_values=[0, 2, 4, 6, -2, -4, -6],
                             ts=[1, 2, 5, 8, 10, 15, 20, 30],
                             model_c='lr', model_t='lr',
                             covariate_model_params={'cov_p': 0.35},
                             est_params_c={'C': 100},
                             effect_types=effect_types,
                             model_names=['nv', 'cf', 'w', 'west'],
        ...
        # setting 1 otherwise)
        res = run_experiment(n_train=5000,
                             event_params={'p_group_0': 0.01, 'p_group_1': 0.1,
                                           'p_a_group_0': 0, 'p_a_group_1': 0, 'support_cov': 0},
                             comp_params={'p_group_0': 0.01, 'p_group_1': 0.01,
                                          'p_a_group_0': 0, 'p_a_group_1': 0, 'support_cov': 0},
