...
def import_twins(
    ...
    c0 = create_competing_risks_from_model(
        x, np.zeros_like(a), t_max, competing_model, competing_model_params
    )
    c1 = create_competing_risks_from_model(
        x, np.ones_like(a), t_max, competing_model, competing_model_params
    )
    c = a * c1 + (1 - a) * c0
...
def run_twins_experiment(
    ...
    support_feats = [
        idx for idx, indicator in enumerate(slct.get_support()) if indicator
    ]
    if not get_support_covs_from_model:
        support_feats = [
            idx
            for idx in np.random.choice(x.shape[1], len(support_feats), replace=False)
        ]
    ...
    if model_c == "cons":
        ...
        model_params_c = {
            "po_estimator": LogisticRegression,
            "solver": "saga",
            "C": 10e-3,
            "penalty": "l1",
            "max_iter": 1000,
        ...
        model_params_c = {
            "po_estimator": LogisticRegressionCV,
            "solver": "saga",
            "cv": 3,
            "penalty": "l1",
            "max_iter": 1000,
            "Cs": [10e-1, 10e-3, 10e-2, 1],
            **est_params_c,
        }
    elif model_c == "boost":
        ...
        model_params_c = {
            "po_estimator": RandomForestClassifier,
            "n_estimators": 30,
            **est_params_c,
        }
    ...
    if model_prop == "lr":
        ...
        model_prop = LogisticRegressionCV(
            **{
                "solver": "saga",
                "cv": 3,
                "penalty": "l1",
                "max_iter": 1000,
                "Cs": [10e-1, 10e-3, 10e-2],
            }
        )
    ...
    for seed in range(n_exp):  # loop over seeds
        for vary_val in vary_vals:  # loop over different experimental effect_types
            ...
            x, t, delta, a, t1, t0, c1, c0, t_max, feat_list = import_twins(
                ...
                treatment_assn_params=treat_assn_model_params,
                competing_model=comp_model,
                competing_model_params=comp_model_params,
                seed=seed,
                scl=scl,
                tmax=tmax,
            )
            ...
            (
                ...
                x, t, delta, a, t1, t0, c1, c0, train_size=n_train, random_state=seed
            )
            ...
            for name in models_to_fit:
                ...
                if name == "nv":
                    ...
                    a_elim = np.random.binomial(
                        1, p=0.5 * np.ones_like(a)
                    )  # remove confounding
                    t_elim = (
                        a_elim * t1 + (1 - a_elim) * t0
                    )  # eliminate competing events
                    ...
                    # oracle estimator fit on counterfactual data
                    # create counterfactual data
                    a_sep = np.random.binomial(
                        1, p=0.5 * np.ones_like(a)
                    )  # remove confounding
                    t_raw_sep = a_sep * t1 + (1 - a_sep) * t0
                    delta_sep = (
                        c0 > t_raw_sep
                    )  # use c0 to separate out effect of a comp event
                    ...
                    # oracle estimator fit on counterfactual data
                    # create counterfactual data
                    a_cs = np.random.binomial(
                        1, p=0.5 * np.ones_like(a)
                    )  # remove confounding only
                    ...
                    if with_prop:
                        _, prop = treat_assn_model(
                            X, return_p=True, **treat_assn_model_params
                        )
            ...
            print(
                "Experiment {}, vary_val {}: Completed all model training.".format(
                    seed, vary_val
                )
            )
                            ...
                            if metric == "rmst":
                                ...
                                if name == "cf":
                                    ess_0, ess_1, n_0, n_1 = model_dict[
                                        ...
                                        t_sep
                                        if setting == "sep"
                                        else (t_elim if setting == "elim" else t_cs),
                                        delta_sep
                                        if setting == "sep"
                                        else (
                                            delta_elim
                                        ...
                                        a_sep
                                        if setting == "sep"
                                        else (a_elim if setting == "elim" else a_cs),
                                        t_horizon,
                                    )
...
def create_competing_risks_from_model(
    X, a, t_max, event_model, event_model_params=None
):
...
def run_experiment_by_setting(
    ...
    if setting == "1":
        ...
        raise ValueError(
            "Twins setting should be 1, 2 or 3; you passed {}.".format(setting)
        )
